...
async def create_bug(
    ...
    # Priority: 1) bug.project_id from frontend, 2) user_story, 3) test_case
    ...
    if not project_id:
        raise HTTPException(
            status_code=400,
            detail="Bug must have project_id or be associated with a user_story_id or test_case_id"
        )
    ...
    # Update test execution with bug_id if execution_id is provided and valid
    if bug.execution_id and bug.execution_id > 0:
        from backend.database.models import TestExecutionDB
        execution = db.query(TestExecutionDB).filter(TestExecutionDB.id == bug.execution_id).first()
        if execution:
            existing_bugs = execution.bug_ids.split(",") if execution.bug_ids else []
            if bug.id not in existing_bugs:
                existing_bugs.append(bug.id)
                execution.bug_ids = ",".join(existing_bugs)
                db.commit()
                print(f"   ✅ Updated execution {bug.execution_id} bug_ids: {execution.bug_ids}")
        else:
            print(f"   ⚠️  Warning: Execution {bug.execution_id} not found, skipping bug_ids update")
    elif bug.execution_id == 0:
        print(f"   ℹ️  Bug reported during active execution (not saved yet), skipping execution update")
